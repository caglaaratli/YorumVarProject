...
function Sidebar({ setContent }) {
  ...
  return (
    <div className={`bg-gray-200 ${isOpen ? "w-64" : "w-20"} transition-width duration-300 h-full`}>
      {isMobile && (
        <button onClick={toggleSidebar} className="text-white p-2 m-2 bg-indigo-500 rounded">
          {isOpen ? '<' : '>'}
        </button>
      )}
      <div className={`flex flex-col space-y-4 p-5 ${isOpen ? "block" : "hidden"}`}>
